...
namespace decryptor
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                byte[] code = GetGame2(keyIv[0], keyIv[1]);
        ...
        public static byte[] GetGame(string key, string iv)
        {
            /*
                         var assembly = Assembly.GetExecutingAssembly();
            string enc_cipher="";
            using (Stream stream = assembly.GetManifestResourceStream("code"))
            {
                using (BinaryReader r = new BinaryReader(stream))
                {
                    enc_cipher = r.ReadString();
                    Console.WriteLine(enc_cipher);
                }
            ...
            aes.Key = textEncoder.GetBytes("x0z1asxLjLX1EWw8WScxyFHIDDWsScp1");
            aes.IV = textEncoder.GetBytes("7msdl3r1TOZzaFs5");
            ...
            return Encoding.ASCII.GetBytes(enc_cipher);
        }
        ...
        public static byte[] GetGame2(string key, string iv)
        {
